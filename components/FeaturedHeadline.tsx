...
import { Startup, SUBCATEGORY_LABELS, CAUSE_LABELS } from "@/lib/types";

export function FeaturedHeadline({ startup }: { startup: Startup }) {
  const teaserParagraphs = startup.content
    .split("\n\n")
    .slice(0, 3)
    ...
    <Link href={`/startup/${startup.slug}`} className="block group">
      <div className="px-8 py-8 text-center border-b border-rule">
        ...
        <p className="text-sm text-ink-muted mt-3 uppercase tracking-[1px]">
          {startup.name} — {SUBCATEGORY_LABELS[startup.aiSubcategory]} —{" "}
          {CAUSE_LABELS[startup.causeOfDeath]}
        </p>
        <div className="text-base text-ink-light mt-4 max-w-md mx-auto leading-relaxed">
